...
PROJECT_DIR=${HOME}/GitHub/localize-demo
...
kubectl create -f ${YAML_DIR}/csr.yaml

echo "Approving CSR - ${CSR_NAME}"
kubectl certificate approve ${CSR_NAME}

echo "Extracting PEM"
kubectl get csr ${CSR_NAME} -o jsonpath='{.status.certificate}' | openssl base64 -d -A -out ${CERTS_DIR}/${APP}.${NAMESPACE}.pem 
